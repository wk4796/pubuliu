...
app.get('/api/images', async (req, res) => {
    ...
    res.json({ images: images.slice(startIndex, endIndex), page: pageNum, totalPages: Math.ceil(images.length / limitNum), hasMore: endIndex < images.length });
});
...
<body class="antialiased text-slate-800">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const DOMElements = {
            navContent: document.getElementById('nav-content'), navStats: document.getElementById('nav-stats'),
            contentView: document.getElementById('content-management-view'), statsView: document.getElementById('statistics-view'),
            tabGallery: document.getElementById('tab-gallery'), tabRecycleBin: document.getElementById('tab-recycle-bin'),
            categoryMgmtSection: document.getElementById('category-management-section'), searchContainer: document.getElementById('search-container'),
            imageList: document.getElementById('image-list'), imageLoader: document.getElementById('image-loader'),
            imageListHeader: document.getElementById('image-list-header'),
            searchInput: document.getElementById('search-input'),
            // ... other DOM elements
        };
        let isInRecycleBinView = false;
        ...
        const showToast = (message, type = 'success') => { /* ... unchanged ... */ };
        
        // --- Navigation ---
        function switchView(viewName) {
            if (viewName === 'stats') {
                DOMElements.contentView.classList.add('hidden'); DOMElements.statsView.classList.remove('hidden');
                DOMElements.navContent.classList.remove('active'); DOMElements.navStats.classList.add('active');
                loadStatistics();
            } else {
                DOMElements.statsView.classList.add('hidden'); DOMElements.contentView.classList.remove('hidden');
                DOMElements.navStats.classList.remove('active'); DOMElements.navContent.classList.add('active');
            }
        }
        DOMElements.navContent.addEventListener('click', () => switchView('content'));
        DOMElements.navStats.addEventListener('click', () => switchView('stats'));
            ...
            try {
                ...
                if (stats.popular_downloads.length > 0) {
                    stats.popular_downloads.forEach(item => {
                        ...
                        li.className = 'flex items-center gap-4 py-2 border-b border-slate-100 last:border-b-0';
                        li.innerHTML = `<img src="${item.thumbnail || 'https://via.placeholder.com/40'}" class="w-10 h-10 rounded object-cover bg-slate-200"><p class="flex-grow text-sm text-slate-700 truncate" title="${item.originalFilename}">${item.originalFilename}</p><p class="text-sm font-semibold text-slate-900">${item.count.toLocaleString()} 次</p>`;
                        popularList.appendChild(li);
                    });
                } else { popularList.innerHTML = '<li class="text-slate-500">暂无下载记录。</li>'; }
            } catch (error) { document.getElementById('statistics-view').innerHTML = '<p class="text-center text-red-500">加载统计数据失败。</p>'; }
        }

        // --- Content Management Logic ---
        let currentCategory = 'all';
        function setActiveTab(tabName) {
            isInRecycleBinView = tabName === 'recycle';
            DOMElements.tabGallery.classList.toggle('active', !isInRecycleBinView);
            DOMElements.tabRecycleBin.classList.toggle('active', isInRecycleBinView);
            DOMElements.categoryMgmtSection.style.display = isInRecycleBinView ? 'none' : 'block';
            DOMElements.searchContainer.style.display = isInRecycleBinView ? 'none' : 'flex';
            loadContent();
        }
        DOMElements.tabGallery.addEventListener('click', () => setActiveTab('gallery'));
        DOMElements.tabRecycleBin.addEventListener('click', () => setActiveTab('recycle'));
        
        async function loadContent() {
            // ... This function combines the previous loadImages and logic for both views
        }
        
        // ... all other JS logic for modals, uploads, categories, image list events ...
